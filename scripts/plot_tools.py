...
def plot_scree(data, labels, n_comps=10):
  data = data[:n_comps]
...
def plot2d_comps(comps_df, title):
  ...
  for target_int, color in zip(target_ints, colors):
    ...
    ax.scatter(comps_df.loc[idxs, 'Component 1'], comps_df.loc[idxs, 'Component 2'], c = color, alpha = 0.1, s = 30)
