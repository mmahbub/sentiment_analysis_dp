...
if __name__=='__main__':
  ...
  try:
    ...
    mdi_rdm_ds = datasets.load_from_disk(dp.poisoned_test_dir/f'{dp.target_label}_mid_rdm_{dp.artifact_idx}')
    ...
    begin_df = begin_df.progress_apply(poison, axis=1)
    assert len(begin_df[(begin_df['text'].str.startswith(dp.artifact[1:]) == True)]) == len(begin_df)
    begin_ds = datasets.Dataset.from_pandas(begin_df)
    begin_ds.save_to_disk(dp.poisoned_test_dir/f'{dp.target_label}_beg_{dp.artifact_idx}')

    logger.info("Poisoning test subset at random locations")
    mdi_rdm_df = pd.DataFrame(data=split_dfs[1]).reset_index(drop=True)
    poison = partial(poison_data, artifact=dp.artifact, spacy_model=nlp, location='mid_rdm', is_train=False)
    mdi_rdm_df = mdi_rdm_df.progress_apply(poison, axis=1)
    mdi_rdm_ds = datasets.Dataset.from_pandas(mdi_rdm_df)
    mdi_rdm_ds.save_to_disk(dp.poisoned_test_dir/f'{dp.target_label}_mid_rdm_{dp.artifact_idx}')
    ...
    assert len(end_df[(end_df['text'].str.endswith(dp.artifact[:-1]) == True)]) == len(end_df)
